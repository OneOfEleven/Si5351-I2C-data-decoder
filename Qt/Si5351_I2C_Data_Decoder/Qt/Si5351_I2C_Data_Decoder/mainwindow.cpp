...
void __fastcall MainWindow::loadSettings()
{
    ...
    settings.beginGroup("Misc");
    {
        ...
        QString s = settings.value("XtalFrequency", ui->RefHzLineEdit->text()).toString();
        ui->RefHzLineEdit->setText(s);

        //const int x = settings.value("SplitterPos", ui->splitter->pos().x()).toInt();
        //ui->splitter->pos().setX(x);
...
void __fastcall MainWindow::saveSettings()
{
    ...
    settings.beginGroup("Misc");
    {
        ...
        //settings.setValue("SplitterPos", ui->splitter->pos().x());
...
bool __fastcall MainWindow::loadFile(QString filename)
{
    ...
    while (!stream.atEnd())
    {
        QString line = stream.readLine().trimmed();						// read a single line of text (excluding '\n') from the settings file
        if (line.isEmpty())
    ...
    for (unsigned int i = 0; i < m_file_lines.size(); i++)
    {
        ...
        if (s.indexOf(QChar('.'), 0, Qt::CaseInsensitive) >= 0)
        {
            bool ok = false;
...
//    const bool     ss_center            = (m_si5351_reg_values[SI5351_REG_SPREAD_SPECTRUM_PARAMETERS_2] & 0x80) ? true : false;
    ...
    {
        if (pll_a_Hz >= 1e6)
            s.sprintf(" %0.7f MHz", pll_a_Hz / 1e6);
        else
            s.sprintf(" %0.4f kHz", pll_a_Hz / 1e3);
...
*/
    ...
    {
        if (pll_b_Hz >= 1e6)
            s.sprintf(" %0.7f MHz", pll_b_Hz / 1e6);
        else
            s.sprintf(" %0.4f kHz", pll_b_Hz / 1e3);
